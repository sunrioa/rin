...
- stable operation ID generated by trusted Host identity state, never by model
  output or a caller-supplied retry value;
...
replay reconstructs Rin state only. After the final report is acknowledged,
HostKit may discard the terminal lifecycle record; the authoritative game
store retains any applied-operation marker required by its durability profile.
