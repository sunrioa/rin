...
`rin-mcp` is Rin's optional external-control gateway. The current release is
read-only:
...
- no current tool can mutate a world.

Write operations, cancellation, operation tracking, and Streamable HTTP MCP are
future phases described in the
...
- An Offer is only a Host-bound action candidate. This phase does not expose an
  Offer execution tool.
