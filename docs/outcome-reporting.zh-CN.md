...
Commit 和每个 Batch Commit Item 的 `accepted` 都是必填字段。拒绝必须显式序列化
`false`；省略或 `null` 不表示拒绝，而是返回 `400 invalid_request`。公共 JSON
请求中的每个整数还必须处于 `-9007199254740991` 至 `9007199254740991` 的精确
跨语言范围，然后继续应用字段自己的更窄约束。
