...
本文记录与引擎无关的 v0.5 实施契约，不定义或跟踪任何单独的使用方游戏。
...
每个新契约都以与引擎无关的方式定义，并在适配器使用前由 Go 测试覆盖。
...
接入方可以只启用 Memory Archive 与 Belief Conflict，而不启用自主候选
Goal 或 Arbitration。后两项能力需要游戏显式提供候选目标和多角色场景。
...
- 允许适配器只启用自身支持的 Memory 和 Belief Feature。
...
- 在游戏创建新的 Rin Session 时启用兼容认知 Feature。
...
当全部自动检查通过、每阶段都有本地 Commit、参考使用方可以启用认知而不
改变 Canon 世界权威，并且只剩 GUI、跨引擎场景、长时间运行和人工质量
检查时，实施完成。
