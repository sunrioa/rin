...
每个候选 `ActionSpec.description` 都是游戏的展示授权边界：游戏必须保证该动作
被选中时，其描述可以安全展示给玩家。不得把私有 Goal、Boundary、Memory、
Belief 或 Prompt 文本放入动作描述。

返回的 Proposal 带：

- `based_on_revision` 和 `based_on_head_hash`：生成依据。
- `action`：原样取自游戏候选动作，Policy 不能添权。本契约只授权展示其中的
  `description`；ID、Kind、Target 与 Parameter 仍是集成数据，除非游戏另行授权。
- `summary`：由 Rin 根据选中的、游戏编写的动作描述重建。
- `rationale`：固定的 stance UI 文案，不是模型推理或角色私有文本。
- `recalled_memory_ids`、`goal_id`、`boundary_id`、`policy_source` 与完整的
  `proposed_goal`：私有结构化审计/集成数据。玩家 UI 不得直接展示；仅
  `boundary_id` 或候选 Goal 是否存在就可能泄露隐藏状态。
- `status: pending`：Rin 尚未收到游戏处理结果；它不是等待 Rin 激活的动作。

`boundary_id` 是可选的 v1 增量响应字段，由运行时在请求 Tag 命中角色边界时推导。
宽容 SDK 解码器必须保留或忽略未知增量字段。它不是模型提供的，也不是展示文案。

对于玩家文本门禁上线前创建的 Event 或 Snapshot，State、Replay、Snapshot 导出
和 exact retry 都返回重建后的玩家字段；原始 append-only Event 或 Restore 中
嵌入的 Snapshot 不会被改写。因此 exact retry 保留原动作、审计 ID、Revision 与
Head，但旧 `summary`/`rationale` 可能升级。
...
`MemorySummary.summary` 是最多 1,000 个 Unicode code point 的确定性有损投影。
每次合并都会预留最旧文本锚点，按重要度分配其余文本预算（近期性用于同类取舍），
并预留最新锚点。重要度会提高在有界投影中的保留机会，但不保证原文永久存在。
每类来源 ID 最多 64 个，保留已知最旧与最新条目，并在中间 tick 范围中取代表样本；
它是代表性证据，不是完整来源 ledger。时间最旧四条直接 Summary 的 merge lineage
与 Summary ID 派生保持稳定，因此旧 Proposal 事件中持久化的
`recalled_memory_ids` 仍能重放。

从 Memory 或 Summary State 移除细节只是 cognition 压缩，不等于删除权威数据，
也不构成隐私擦除。`events.jsonl`、Identifier History、备份、checkpoint 及仍被
保留的 Snapshot 可能继续包含底层文本或 ID。真正的擦除请求必须走部署方另外治理的
删除与备份流程。
