...
Session lineage 增长；超过 inline 上限的 lineage 在计划中的 Step 5 streaming
transport 可用前，不能使用 Snapshot 或 Replay JSON 传输。成功响应：
...
返回原始结果。超限旧事件仍可从磁盘 Open 和 replay，但在计划中的 Step 5
streaming transport 提供前不能通过 inline API 重新传输。
...
一旦超过 16 MiB，就不能 inline 返回或 Restore。完整传输大型 lineage 需要
计划中的 Step 5 streaming transport；Identifier History 绝不会被静默截断。
