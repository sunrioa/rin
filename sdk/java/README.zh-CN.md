...
Java 17 source-first SDK 包含四部分：
...
`YourJsonCodec` 是占位类型，不是 Rin 提供的类。游戏应通过已经使用的 JSON 库实现
`JsonValueCodec`，因此 Rin 不额外绑定 Jackson、Gson 或引擎专用序列化器。Control
Client 使用标准 `java.net.http.HttpClient`，禁止 Redirect，限制响应体和超时，并
