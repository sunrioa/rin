...
func defaultGameOutcomeStatePath() string {
	...
	if err != nil || configDir == "" {
		return "rin-recovery-example-state.json"
	}
	return filepath.Join(configDir, "rin", "recovery-example-state.json")
}
...
func stateDirectorySyncJournalName(statePath string) string {
	...
	return fmt.Sprintf(".rin-recovery-dir-sync-%x.json", sum)
}
