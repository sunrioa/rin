...
func TestAgentRuntimeUsesRealActionGatewayAndHostOutcome(t *testing.T) {
	...
	service := controlplane.New(controlplane.Options{ActionHost: actionHost, PolicyEngine: engine})
	...
	runtime, err := cognition.NewAgentRuntime(cognition.AgentRuntimeOptions{
		...
		MaxAdvancesPerRun: 2, OperationWaitMillis: 1,
	})
	if err != nil {
		t.Fatal(err)
	}
	started, err := runtime.StartTask(context.Background(), cognition.StartTaskInput{
		...
		Goal: "Move near the owner.", Tags: []string{"task.follow"},
	})
	if err != nil {
		t.Fatal(err)
	}
