...
此仓库已gg
吐槽
呃呃呃呃可行度不高 或许适合一个组织来统一管理自己旗下的所有游戏 通用有框架或许不如ai从头开始搓🤯🤯🤯
或许ai游戏可用使用 minimax-h3-world 根据用户实时输入来渲染世界 当然要h3-max的速度 至于h3的另外一个版本或许可以作为技能？时停的时候让人物自由穿梭 在当前帧场景的时候 模型id忘了（（（（（（（（（
至于jev 感觉有可能用来做galgame？ 直接写组件 然后外挂一个实时剧情生成的llm jev判断下一幕用什么组件来渲染哪些画面组件？ 有可能能做到对话人心情不好一下把对话框抓碎并停止下一幕生成 或者病娇强制爱 ？？ 或者用于实时世界生成  yy中（（（（（（（（
